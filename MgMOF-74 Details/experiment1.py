...
MOF_FILENAME = 'C:/Users/Cameron/Documents/MOF files/MgMOF-74.cif'
...
MIN_SAFE_DISTANCE = 0.8  # Critical threshold for steric clash removal (allows LAMMPS to run)

# Creates our electrolyte molecules and applies random rotation around a random 3D axis
...
# Calculates where molecules and Na ions should be placed
...
# Build Shell/Gas Components (with random jitter)
...
# Built system and output its metrics
...
# LAMMPS Setup + Steric Clash Filtering
...
if initial_min_dist < MIN_SAFE_DISTANCE:
    ...
    for i in range(len(system)):
        ...
        # Check MOF atoms: Always keep them
        if i in mof_indices_in_system:
            atoms_to_keep.append(i)
            continue
            
        # Check Mobile atoms (PEO, IL, Na+, CO2):
        ...
        if len(kept_positions) > 0:
            ...
            if kept_dist < MIN_SAFE_DISTANCE:
                # Atom is clashing with another kept atom so remove it
                continue
